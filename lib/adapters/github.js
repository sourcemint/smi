...
exports.download = function(url, path, callback) {
	...
	if (pathParts[2] !== "archive") {
		return callback(new Error("Cannot parse github URL '" + url + "'! It must look something like 'https://github.com/<owner>/<repo>/archive/<hash>.tar.gz'"));
	}
	...
	var command = "git clone git@github.com:" + pathParts[0] + "/" + pathParts[1] + '.git "' + path + '"';

	console.log(("Running command: " + command + " (cwd: " + PATH.dirname(path) + ")").magenta);
