...
function requestForDescriptor(descriptor, requestOptions, callback) {
	if (typeof requestOptions === "string") {
		requestOptions = {
			url: requestOptions
		};
	}
	function returnNoProxy(callback) {
...
exports.resolveDescriptor = function(descriptor, options, done) {
	...
	function resolveLocators(type, locators, mergeHandler, callback) {
...
//			return exports.resolveDescriptor(_descriptor, options, callback);
	...
	if (
		...
		return resolveLocators("extends", locators, function(ourDescriptor, externalDescriptor) {
			...
			if (
				...
				delete descriptor.extends;
				return resolveLocators("overrides", locators, function(ourDescriptor, externalDescriptor) {
					for (var name in externalDescriptor) {
						if (ourDescriptor[name]) {
					        ...
					        function ensureDownloaded(archivePath, url, callback) {
					            return FS.exists(archivePath, function(exists) {
					                ...
					                try {
					                    ...
					                    requestForDescriptor(snapshotDescriptor, url, function(err, response) {
					                        if (err) return callback(err);
