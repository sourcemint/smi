		...
		function ensure(callback, verify) {
			...
			function download(meta, callback) {
                if (/^\./.test(locator)) {
                	...
                	var fromPath = PATH.join(snapshotDescriptor._path, "..", locator);
                	return etagForPath(fromPath, function(err, etag) {
		                ...
		                console.log(("Copy catalog for upstream alias '" + id + "' from '" + locator + "'").magenta);
                ...
                if (/^https?:\/\//.test(locator)) {
	                console.log(("Download catalog for upstream alias '" + id + "' from '" + locator + "'").magenta);
	                return REQUEST({
	                    method: "GET",
	                    url: info.url,
	                    headers: {
...
console.log("response", response);
	            ...
	            return callback(new Error("Cannot determine how to download '" + locator + "'!"));
			}
	...
	function ensurePackage(id, locator, callback) {
		...
		console.log(("Ensure package: " + id).cyan);
			...
			return FS.exists(_abspath(locator.smi.installedPath), function(exists) {
				...
				return FS.rename(_abspath(locator.smi.extractedPath), _abspath(locator.smi.installedPath), function(err) {
					...
					return require.async("./adapters/" + type, function(api) {
						return api.install(basePath, locator, function(err) {
							if (err) return error(err);
							return callback(null);
						});
					}, error);
