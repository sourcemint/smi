					            ...
					            return FS.exists(archivePath, function(exists) {
					                ...
					                if (/^\//.test(url)) {
					                	return FS.exists(url, function(exists) {
					                		if (!exists) {
					                			return callback(new Error("Cannot map package '" + id + "' to '" + url + "' as path does not exist!"));
					                		}
						                    console.log(("Linking '" + url + "' to '" + archivePath + "'").magenta);
						                    if (!FS.existsSync(PATH.dirname(archivePath))) {
						                    	FS.mkdirs(PATH.dirname(archivePath));
						                    }
						                	return FS.symlink(PATH.relative(PATH.dirname(archivePath), url), archivePath, callback);
					                	});
					                }
