...
exports.install = function (basePath, snapshotDescriptor, callback) {
	...
	var repeatEnsureAfterInstall = false;
			...
			if (
				...
				locators.forEach(function(locator, i) {
					return waitfor(function(callback) {
						if (/^\./.test(locator)) {
							var path = PATH.join(descriptor._path, "..", locator);
							return FS.exists(path, function(exists) {
								if (!exists) {
						    ...
						    hash.update(locator);
							var extendsPath = descriptor._path + "~extends~" + hash.digest("hex").substring(0, 7);
							...
							return FS.exists(extendsPath, function(exists) {
								...
								function checkIfPackageSymlink() {
									...
									return FS.exists(packagePath, function(exists) {
										...
										return FS.exists(path, function(exists) {
											...
											console.log(("Linking '" + path + "' to '" + extendsPath + "'.").magenta);
	...
	function prepare(callback) {
		...
		return ensureDescriptor(function(err, snapshotDescriptor) {
			...
			function ensurePackage(id, locator, callback) {
				...
				if (repeatEnsureAfterInstall && id !== repeatEnsureAfterInstall) {
					return callback(null);
				}
				...
				try {
					...
					function ensureAspect(targetPath, aspect, locator, options, callback) {
						...
						var desiredLocator = {
							...
							install: (typeof locator.install !== "undefined") ? locator.install : (aspect === "source"),
						...
						if (!desiredLocator.location) {
							return callback("No 'location' specified for package: " + id);
						}
			...
			return ensure(function(err) {
				...
				return link(function(err) {
					...
					return install(function(err) {
						if (err) return callback(err);
						if (repeatEnsureAfterInstall) {
							repeatEnsureAfterInstall = false;
