...
var SMI_HOME = process.env.SMI_HOME || (process.env.HOME && PATH.join(process.env.HOME, ".smi")) || null;
if (!SMI_HOME) {
	throw new Error("'SMI_HOME' or 'HOME' environment variable must be set!");
}
...
exports.install = function (basePath, snapshotDescriptor, installOptions, callback) {
	...
	if (installOptions.debug) {
		FS.on("used-path", function(path, method) {
			console.log(("FS." + method).yellow.bold, path);
		});
	}
...
*/
					...
					return FS.exists(installedPath, function(exists) {
						...
						function writePackageDescriptor(callback) {
							if (!locator.descriptor) {
								return callback(null);
							}
							var packageDescriptorPath = PATH.join(installedPath, "package.json");
							return FS.exists(packageDescriptorPath, function(exists) {
								if (exists) {
									packageDescriptorPath = packageDescriptorPath.replace(/\.json$/, ".1.json");
								}
								...
								return FS.outputFile(packageDescriptorPath, JSON.stringify(locator.descriptor, null, 4), callback);
							});
						}
						...
						if (locator.symlinked) {
							...
							return copy(function(err) {
								...
								return writePackageDescriptor(function(err) {
									if (err) return error(err);
