					...
					try {
					        ...
					        function ensureDownloaded(archivePath, url, callback) {
					            return FS.exists(archivePath, function(exists) {
					                ...
					                try {
					                    ...
					                    return requestForDescriptor(snapshotDescriptor, url, function(err, response) {
					                        if (err) return callback(err);
					                    	...
					                    	return req.pipe(FS.createWriteStream(tmpPath));
					                    });
					                } catch(err) {
					                    return callback(err);
					                }
						...
						if (locator.aspects) {
							...
							for (var aspect in bestAspects) {
								waitfor(targetPath, aspect, bestAspects[aspect], {
