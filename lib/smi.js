...
function requestForDescriptor(descriptor, requestOptions, callback) {
	...
	if (
		...
		return DNS.resolve4("a.domain.that.will.never.resolve.so.we.can.determine.default.ip.com", function(err, defaultInfo) {
			if (err) return callback(err);
			var proxyParts = URL.parse(descriptor.config["smi.cli"].proxy);
			return DNS.resolve4(proxyParts.hostname, function(err, info) {
				if (err) return callback(err);
				if (
					info && info.length === 1 &&
					(
