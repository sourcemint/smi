									...
									extractor = function(archivePath, targetPath, callback) {
						                ...
						                return EXEC("tar --version", {
											...
											if (extension === "tar.bz2") {
												command = 'tar -xjf "' + PATH.basename(archivePath) + '"' + strip + ' -C "' + targetPath + '/"';
											} else {
												command = 'tar -xzf "' + PATH.basename(archivePath) + '"' + strip + ' -C "' + targetPath + '/"';
											}
											if (locator.subpath) {

												if (/bsdtar/.test(stdout)) {
													command += ' "*/' + locator.subpath + '"';
												} else
												if (/GNU tar/.test(stdout)) {
													command += ' --wildcards "*/' + locator.subpath + '"';
												} else {
													console.error("stdout", stdout);
													console.error("stderr", stderr);
													return callback(new Error("'tar' flavour not supported!"));
												}
											}
											if (installOptions.verbose) {
			...
			function doit(id, info, callback) {
				...
				if (
	                ...
	                return EXEC(command, {
				        ...
				        return FS.copy(info.aspects[''].location, livePath, callback);
	                });
				} else
				if (
					info.aspects &&
